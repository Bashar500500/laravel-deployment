...
class LearningActivityContentUploadRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'audio' => ['required_if:content_type,==,Video', 'missing_if:content_type,==,' . implode(',', LearningActivityType::Audio->getEnumsExceptValue()), 'file', 'mimes:mp3,wav,ogg,m4a'],
            'word' => ['required_if:content_type,==,Video', 'missing_if:content_type,==,' . implode(',', LearningActivityType::Word->getEnumsExceptValue()), 'file', 'mimes:doc,docx'],
            'power_point' => ['required_if:content_type,==,Video', 'missing_if:content_type,==,' . implode(',', LearningActivityType::PowerPoint->getEnumsExceptValue()), 'file', 'mimes:ppt,pptx'],
            'zip' => ['required_if:content_type,==,Video', 'missing_if:content_type,==,' . implode(',', LearningActivityType::Zip->getEnumsExceptValue()), 'file', 'mimes:zip'],
